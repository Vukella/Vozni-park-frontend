...
export function BrandsPage() {
    ...
    const columns: Column<BrandResponse>[] = useMemo(
        () => [
            ...
            {
                key: 'vehicleModelCount',
                header: 'Models',
                sortable: true,
                // FIXED: was b.modelCount — correct field is vehicleModelCount
                render: (b) => b.vehicleModelCount ?? '—',
            },
        ],
        []
    );
    ...
    return (
        <div>
            ...
            <DataTable
                columns={columns}
                data={filtered}
                keyExtractor={(b) => b.idBrand}
                loading={loading}
                emptyMessage="No brands found."
                actions={(b) => (
                    <>
                        <button onClick={() => { setEditing(b); setFormOpen(true); }} className="btn-icon" title="Edit">
                            <Pencil className="h-4 w-4" />
                        </button>
                        <button onClick={() => setDeleteTarget(b)} className="btn-icon text-red-400 hover:text-red-600 hover:bg-red-50" title="Delete">
            ...
            <ConfirmDialog
                ...
                title="Delete Brand"
                message={deleteTarget ? `Delete "${deleteTarget.brandName}"? Associated vehicle models will also be affected.` : ''}
            />
        </div>
    );
}